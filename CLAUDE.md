...
- **Commands** (`.claude/commands/`): `commit`.
- **Always-active rules** (`.claude/rules/`): `tenant-isolation`, `contracts-before-code`, `spec-first`, `no-plaintext-clinical-data`.
...
_Pending until the Phase 0 scaffold exists (see `plan.md` §8)._ Once it does: `npm install`, environment variables in `.env.local` (never committed), `npm run dev`, migrations with `npx prisma migrate dev`.
