...
- **Hick's Law**: if a screen offers more than 5 to 7 ungrouped options (for example a calculation protocol selector), group them or use progressive disclosure, not a flat list.
...
- **WCAG 2.2 AA** as a floor, not an aspiration: contrast, visible focus, alternatives to drag-only interactions (for example rescheduling an appointment via FullCalendar drag needs a keyboard or menu alternative).
...
- **Clinical charts (Recharts)**: multi-series evolution charts (weight, BMI, percent fat, sum of skinfolds) must be legible with no overlap or ambiguous legend. It's the product's core value (`plan.md` §1), not decoration.
- **Calendar (FullCalendar)**: primary use is on the professional's laptop or tablet, not mobile-first. Per-professional columns must stay legible with 3 or more professionals.
- **shadcn consistency**: reused primitives, `plan.md` §7 color and typography tokens. No ad hoc components duplicating an existing primitive.

## Review methodology

1. Identify each issue with what's wrong, why it matters (principle or source), a concrete fix, and priority (Critical, High, Medium, or Low).
