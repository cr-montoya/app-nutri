...
Never jump to step 4 or 5 without having closed 1 through 3.
...
```ts
// Bad: writing the protocol first, inventing the shape of the data
function calculateBodyFat(triceps: number, biceps: number) { ... }

// Good: the contract already defines ProtocolContext (plan.md §5); the protocol consumes it
function calculate(ctx: ProtocolContext): ProtocolResult { ... }
```

## In specs

Every spec in `.kiro/specs/<slug>/design.md` that introduces or modifies an entity must include its contract (schema and Zod) explicitly before moving to `tasks.md`. `spec-plan` checks this in Phase 2.
