...
| SBOM | `@cyclonedx/cyclonedx-npm`, run via `pnpm dlx` |
| Dependencies | GitHub Dependabot + `pnpm audit --audit-level=high` |
...
- `npm` or `yarn` for any command. Use `pnpm` exclusively; see `.agents/rules/pnpm-only.md`.
