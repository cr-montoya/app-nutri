...
- [x] T7.1 Connect the Vercel project to the GitHub repo and configure the Neon Vercel integration for per-PR database branching. Validation: PR #1 received a successful Vercel Preview deployment and the user verified its corresponding Neon branch exists. Closes REQ-019.
- [x] T7.2 Update `src/lib/db.ts` to resolve the runtime connection from Vercel's dynamic `DATABASE_URL` when `VERCEL_ENV` is set, retaining fail-closed `APP_DATABASE_URL` behavior locally; add unit coverage for both paths. Validation: `pnpm test -- db`. Closes REQ-022. 59/59 tests passing; `pnpm exec tsc --noEmit` also passes.
- [x] T7.3 Validate the Neon-Vercel integration uses `appnutri_app`, redeploy this PR, then confirm its preview serves `/register` and its database branch is distinct from the base branch. Validation: commit `c8b60a8` deployed successfully at `https://app-nutricionista-deeeau13w-romanops.vercel.app`; the user logged in through the protected preview, confirmed the Neon branch exists, and confirmed the integration role is `appnutri_app`. Closes REQ-019, REQ-023, REQ-024.
...
## T9: Preview security gates (REQ-025 through REQ-029)

- [x] T9.1 Add `.github/workflows/dast.yml` and `.zap/rules.tsv`: run commit-pinned OWASP ZAP Baseline only after a successful internal Vercel Preview deployment status, pass the Vercel bypass through ZAP's header environment variables, disable GitHub issue writing, and upload scan reports. Validation: `ruby -e 'require "yaml"; YAML.load_file(".github/workflows/dast.yml")'` and `git diff --check` passed; remote validation is T9.4. Closes REQ-025, REQ-026, REQ-027.
- [x] T9.2 Replace the broken SBOM script contract with pnpm's built-in CycloneDX command; add a pull-request SBOM artifact job to `security.yml`, ignore generated artifacts, and update README commands to pnpm only. Validation: `mkdir -p artifacts && pnpm sbom --sbom-format cyclonedx > artifacts/sbom.cdx.json && jq -e 'type == "object"' artifacts/sbom.cdx.json && git check-ignore artifacts/sbom.cdx.json` passed. Closes REQ-028, REQ-029.
- [x] T9.3 Record the completed Neon positive and negative RLS validations in `design.md`. Validation: `pnpm test -- rls-positive && pnpm test -- rls-negative` passed (59 tests each). Closes REQ-029.
- [x] T9.4 Push the approved CI changes to the feature branch and verify the Preview deployment plus DAST, SBOM, dependency review, gitleaks, and Semgrep checks through GitHub. Validation: `gh pr checks 1 --watch --interval 10` passed; DAST run `32942691999` succeeded. Closes REQ-025, REQ-026, REQ-027, REQ-028.
