...
- [ ] T7.3 Validate the Neon-Vercel integration uses `appnutri_app`, redeploy this PR, then confirm its preview serves `/register` and its database branch is distinct from the base branch. Validation: Vercel deployment is `Ready`; request `<preview-url>/register` returns 200; Neon integration metadata shows role `appnutri_app` and a branch for PR #1. Closes REQ-019, REQ-023, REQ-024.
