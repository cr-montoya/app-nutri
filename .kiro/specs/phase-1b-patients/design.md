...
- **`plan.md` §4's `AuditLog` "mandatory" entity list updated to include `Patient`** (both the write and detail-view-read cases). A `reviewer` gate finding: REQ-021 already explained why `Patient` extends that list (`plan.md` §8's Phase 1 description), but §4's own sentence hadn't been updated to match, leaving the architecture source of truth slightly stale against what this spec actually implements and requires.
- **The RLS migration's code comment corrected**: it previously claimed `design.md` shows an explicit `WITH CHECK` clause; `design.md`'s RLS section only shows `USING`. The migration's `WITH CHECK` (functionally a no-op addition for a `FOR ALL` policy, since Postgres defaults an omitted `WITH CHECK` to `USING`) actually follows the `invites` migration's pattern instead, not `design.md`'s. A `reviewer` gate LOW finding, comment-only fix.
- **`.kiro/steering/tech.md` now notes the `pg_trgm` extension** under the Postgres row, since it's this project's first non-default Postgres extension (per this document's own "Reused vs. new" section above) and wasn't recorded anywhere durable outside this spec's migration comments. A `reviewer` gate LOW finding; judged below the `.agents/rules/adr-required.md` bar for a full ADR (no genuinely competing alternative for indexed leading-wildcard case-insensitive search in Postgres), so a steering-doc note rather than an ADR.
- **A `database-architect` gate ran after the fact**, closing a gap the `reviewer` gate flagged: the schema/RLS/migration work in T1 was done by directly applying `database-architect.md`'s checklist during implementation (visible in the migration/schema comments), not by a separate independent pass, which `docs/testing-and-security.md`'s gate matrix requires without a "consult only" qualifier for this change type. The independent pass ran clean: verified the nullable-unique-constraint behavior and the RLS policy (including fail-closed-with-no-session-variable-set and cross-org-write-rejection cases) via direct `psql` queries against the real database, and confirmed the `pg_trgm`+GIN choice serves REQ-017's query shape via `EXPLAIN ANALYZE` against a 60k-row synthetic dataset. No schema or migration changes resulted.
