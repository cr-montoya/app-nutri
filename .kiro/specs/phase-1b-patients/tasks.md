...
- [ ] T3.1 Write `src/validation/patients.ts`: the shared Zod schema (`fullName`, `phone`, `documentId`, `birthDate`, `sex`, `email`, `address`) with all the bounds and format rules from `requirements.md`. Validation: `pnpm test -- validation`. Closes REQ-002, REQ-003, REQ-004, REQ-008, REQ-009, REQ-010, REQ-011.
