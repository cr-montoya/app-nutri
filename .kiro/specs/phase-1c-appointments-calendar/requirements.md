...
- **REQ-024**: WHEN a member reschedules an `Appointment` through a non-drag interface (an edit form or menu reachable by click/tap and by keyboard, not only by dragging the calendar event), THE SYSTEM SHALL apply the same rules as REQ-011/REQ-012. This requirement exists so that dragging (REQ-013) is never the only way to perform a reschedule, per WCAG 2.2 SC 2.5.7 (Dragging Movements): every drag-based operation needs a single-pointer, non-drag equivalent.
- **REQ-025**: WHEN a member views the calendar, THE SYSTEM SHALL distinguish each individual `AppointmentStatus` value (`SCHEDULED`, `CONFIRMED`, `COMPLETED`, `CANCELLED`, `NO_SHOW`) using at least one visual channel other than color alone (for example an icon or a text label), so status is legible without depending on color perception, per WCAG 2.2 SC 1.4.1 (Use of Color). This sharpens REQ-020's two-bucket (active/terminal) minimum into a per-status requirement.
- **REQ-026**: WHEN a create, reschedule (REQ-006), edit (REQ-011/REQ-012), or status-transition (REQ-017) request is rejected, THE SYSTEM SHALL surface the specific rejection reason to the member at the point of interaction (inline on the form, or as an inline error near the calendar for a drag-based attempt), AND WHEN the rejected request originated from a calendar drag, THE SYSTEM SHALL revert the dragged event to its prior slot without persisting the attempted change.
- **REQ-027**: WHEN a member views the calendar for a professional/day/range with zero appointments, THE SYSTEM SHALL display an explicit empty state (for example "No appointments scheduled") rather than an unlabeled blank grid.
- **REQ-028**: THE SYSTEM SHALL ALWAYS render every interactive calendar element (an event chip, a status-transition button, an edit-sheet button) with a minimum 44×44px hit area, verified with 3 or more professional columns visible, per WCAG 2.2 SC 2.5.8 (Target Size Minimum) and Fitts's Law for a tool used on tablet during a live consultation.
