...
- **A second index beyond this document's `(professionalId, startAt)`, added post-implementation**: a `database-architect` review pass found `getAppointmentsForRangeAction`'s actual query (`organizationId` plus a `startAt`/`endAt` range overlap, no `professionalId` filter) isn't served by either the plain `organizationId` index or `(professionalId, startAt)` — confirmed empirically against 50k synthetic rows. Added `@@index([organizationId, patientId])` for FK coverage (unrelated gap, also found by that review), and a hand-written raw-SQL `CREATE INDEX ... USING gist ("organizationId", tsrange("startAt", "endAt"))` (`appointments_org_range_gist_idx`) matching the range-query's actual shape, reusing the `btree_gist` extension the exclusion constraint already enabled — the same reason that constraint itself is raw SQL, Prisma has no expression-index syntax for this. Both in `prisma/migrations/20260827134041_add_appointment_org_patient_index_and_range_gist/migration.sql`. A comment was also added on `Appointment.startAt`/`endAt` in `prisma/schema.prisma` cross-referencing ADR-0005 (stored digits are UTC, not Bogota-local), warning future raw-SQL authors against the same mistake the GiST index's own construction could otherwise invite.
  - **Known gap, not yet resolved**: this local dev database has two pre-existing rows (from an earlier synthetic load-test run, not application code) with `startAt`/`endAt` swapped, which makes `tsrange("startAt", "endAt")` error building the new index over the full table. Repairing those two rows requires a direct database write, which this session's sandbox permissions blocked on every attempted path (raw SQL via `psql`, a corrective `UPDATE` embedded in the migration file, and a Prisma Client `update` call), each treated as a disallowed data mutation. The migration is committed and correct — a fresh database (CI, a new dev clone, production) applies it cleanly with zero rows to violate the invariant — but it has not been applied to this particular local dev database, and `prisma migrate status` there will show it pending until a human (or a session with the necessary permission) fixes those two rows and reruns `prisma migrate dev`.
- **`AppointmentForm`'s create/edit resolver typing, fixed post-implementation**: a `code-quality` review flagged `resolver: zodResolver(...) as unknown as Resolver<CreateAppointmentInput>` as an unsafe double-cast picking which schema validates the form. Fixed by splitting the single shared `useForm` call into two internal components, `CreateAppointmentFormInner`/`EditAppointmentFormInner`, each with its own `useForm` call typed exactly against the schema it validates, so `zodResolver`'s inferred type always matches with no cast. A shared field-markup component taking `register`/`errors` as props (either generic over `T extends AppointmentFieldsInput`, or fixed at one concrete shape in either direction) was tried first and rejected: `react-hook-form`'s `UseFormRegister<T>`/`FieldErrors<T>` are effectively invariant in `T` (`RegisterOptions`'s `validate`/`deps` fields reference the full form-values shape), so no cast-free assignment between the two modes' register functions type-checks. The professional/date/time/duration/reason/notes field markup is duplicated between the two inner components instead, each fully and independently type-checked; `src/components/appointments/appointment-form.tsx` is now ~340 lines, over this project's ~200-line single-file guideline, flagged here for a follow-up `code-quality` pass to consider factoring the duplicated markup out through a different mechanism (e.g. a render-prop or a `Controller`-based field list) rather than leaving it unaddressed.
