...
- **`DateTime` maps to `timestamp(3)` without timezone, not `timestamptz`.** This document's schema section claimed `timestamptz`, but Prisma's actual default for `DateTime` (verified against the generated migration) is `timestamp(3)`, and every other model in `prisma/schema.prisma` (`Patient`, `Invite`, etc.) already uses that same mapping. Kept consistent with the existing project-wide convention rather than introducing `@db.Timestamptz` uniquely for `Appointment`; behavior is unaffected, since the app never sets a non-UTC Postgres session timezone and always serializes as UTC at the JS `Date` boundary.
- **Two files not in this document's "Files to create or update" table**: `src/server/services/appointments.ts` (`listSchedulingOptions`, the org-scoped patient/professional lists the create/edit form's `<select>`s need — REQ-008's "the UI's own organization-scoped selection lists" cannot render without one) and `src/components/appointments/status-meta.tsx` (the icon/label/color mapping factored out so the calendar's event chips and the detail sheet's status buttons share one definition instead of duplicating REQ-025's mapping twice). Both are small, uncontroversial extensions of patterns this design already established elsewhere (`src/server/services/patients.ts`; the shared `allowedNextStatuses` precedent).
- **`src/lib/db.ts`'s `TENANT_SCOPED_MODELS` set** needed `"Appointment"` added; not called out explicitly in this document's coverage table but required for `withTenant` to accept the model at all (`.agents/rules/tenant-isolation.md`).
- **`updateAppointmentAction` does not re-check REQ-004 (past `startAt`)**, only REQ-003 and REQ-006 through REQ-010 as `requirements.md`'s REQ-011 literally lists. Caught by `tests/e2e/appointment-detail-edit.spec.ts` actually running against real wall-clock time: editing an appointment scheduled earlier the same day failed with "can't be in the past" before this fix, which would have made same-day appointments' reason/notes uneditable once their start time elapsed. Fixed by adding a `checkPast` flag to `resolveAppointmentRange` (`src/validation/appointments.ts`), defaulted on for `createAppointmentAction` and off for `updateAppointmentAction`.
- **T4.9 (REQ-028 touch-target check) is implemented as an automated Playwright test at a tablet viewport size, not a manual check**, once written — `tasks.md` assumed this wasn't automatable in CI, but asserting a rendered element's bounding-box dimensions at a fixed viewport is a standard Playwright assertion; automating it is strictly more repeatable than a manual pass and removes the "documented in the PR description" trust requirement `tasks.md` otherwise relied on.
- **A second index beyond this document's `(professionalId, startAt)`, added post-implementation, then corrected**: a `database-architect` review pass found `getAppointmentsForRangeAction`'s actual query (`organizationId` plus a `startAt`/`endAt` range overlap, no `professionalId` filter) isn't served by either the plain `organizationId` index or `(professionalId, startAt)` — confirmed empirically against 50k synthetic rows. Added `@@index([organizationId, patientId])` for FK coverage (unrelated gap, also found by that review; kept as-is, still correct). The first attempt at the range-query index itself was wrong: a hand-written raw-SQL `CREATE INDEX ... USING gist ("organizationId", tsrange("startAt", "endAt"))` (`appointments_org_range_gist_idx`), on the theory that it matched the range query's shape. A second, independent re-verification pass (`code-quality` and `database-architect` both, empirically) found this GiST index was never actually used: `getAppointmentsForRangeAction` issues a plain Prisma `findMany` with separate `startAt: { lt }, endAt: { gt }` comparisons, never a `tsrange(...) && tsrange(...)` expression, and Postgres can only use a GiST *expression* index when the query literally contains that expression — confirmed at 280k synthetic rows: seq scan, 29ms, 5863 buffers, index never chosen. Replaced in `prisma/migrations/20260827142542_replace_appointment_gist_with_btree_index/migration.sql` with `@@index([organizationId, startAt])`, a plain composite btree matching the query's real shape — no application-code change needed, no raw SQL needed either (unlike the GiST attempt, Prisma's index DSL represents this directly). Confirmed via `EXPLAIN (ANALYZE, BUFFERS)` at 280k synthetic rows: `Bitmap Index Scan on "appointments_organizationId_startAt_idx"`, 0.19-1.2ms, no seq scan. The `btree_gist` extension and the `no_overlapping_active_appointments` EXCLUDE constraint are unaffected — that GiST index and this range-query index were always separate, only the latter was wrong. The prior "known gap" here (two pre-existing local-dev rows with `startAt`/`endAt` swapped blocking the GiST index's build) is moot: those rows don't affect a plain btree index, and this migration applies cleanly. A follow-up finding on this same index, that it's reliably chosen only on a "warm" query plan and its behavior under Neon's actual pooled-connection ("cold") pattern is unverified, is recorded in `docs/adr/0006-appointment-range-query-index-cold-plan-tradeoff.md` rather than here, since it's a decision (keep the index, document a re-benchmark trigger) rather than an implementation detail.
- **`AppointmentForm`'s create/edit resolver typing, fixed post-implementation, then de-duplicated**: a `code-quality` review flagged `resolver: zodResolver(...) as unknown as Resolver<CreateAppointmentInput>` as an unsafe double-cast picking which schema validates the form. Fixed by splitting the single shared `useForm` call into two internal components, `CreateAppointmentFormInner`/`EditAppointmentFormInner`, each with its own `useForm` call typed exactly against the schema it validates, so `zodResolver`'s inferred type always matches with no cast. That first fix traded the cast for ~55 lines of duplicated field markup between the two components, flagged by that same review as needing a follow-up pass. A second `code-quality` pass found the actual root cause and a fix avoiding both problems: react-hook-form's `UseFormRegister<T>` (the un-invoked function) is what's invariant in `T` (`RegisterOptions`'s `validate`/`deps` fields reference the full form-values shape) — but the *return value* of calling `register("fieldName")`, `UseFormRegisterReturn`, carries no such reference and isn't parameterized by `T` at all. Extracted `AppointmentSharedFields`, a presentational component taking the bound `register("fieldName")` results (typed `UseFormRegisterReturn`) and per-field errors (typed as a narrowed `{ message?: string }`, not the full `FieldError<T>`) as props, instead of `register`/`errors` themselves. Each `*Inner` component still calls its own exactly-typed `register`/`formState` and passes the bound results down; no cast anywhere, verified with `pnpm exec tsc --noEmit`. The duplicated ~55-line field-markup block is gone — it exists once now, in `AppointmentSharedFields` — but `src/components/appointments/appointment-form.tsx` is ~365 lines, slightly more than the ~340 it was before this fix: the file trades a duplicated block for a new component's prop interface plus documentation of why the split exists. Still over this project's ~200-line single-file guideline; flagged here again rather than claimed as resolved by line count alone.
